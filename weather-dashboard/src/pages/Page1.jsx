...
import {  fmtVal, fmtTime, aqiColor } from '../utils/helpers';
...
export default function Page1({ location }) {
  ...
  // Extract hourly for the selected date
        ...
        <>
          ...
          <div className="glass-card p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div className="flex items-center gap-4">
              <span className="text-6xl sm:text-7xl" style={{ filter: 'drop-shadow(0 0 16px rgba(0,229,255,0.4))' }}>
                {weatherMeta.icon}
              </span>
              <div>
                <h2 className="temp-hero text-5xl sm:text-6xl neon-text">
                  {fmtVal(currentTemp, 1)}
                  <span className="text-xl sm:text-2xl text-text-muted ml-1">°C</span>
                </h2>
                <p className="text-sm text-text-primary mt-1" style={{ fontFamily: "'Exo 2',sans-serif" }}>
                  {weatherMeta.label}
                </p>
              </div>
            </div>
          ...
          {/* Weather metrics */}
          <div>
            <h3 className="text-xs font-semibold tracking-widest uppercase text-text-muted mb-3" style={{ fontFamily: "'Exo 2',sans-serif" }}>
              Weather Parameters
            </h3>
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3">
              <MetricCard icon="🌡️" label="Max Temp"     value={fmtVal(maxTemp, 1)}  unit="°C" accent="#ff6b6b" />
              <MetricCard icon="🌡️" label="Min Temp"     value={fmtVal(minTemp, 1)}  unit="°C" accent="#b388ff" />
              <MetricCard icon="🌡️" label="Current Temp" value={fmtVal(currentTemp,1)} unit="°C" accent="#00e5ff" />
              <MetricCard icon="🌧️" label="Precipitation" value={fmtVal(precip, 1)}  unit="mm" accent="#f48fb1" />
              <MetricCard icon="🌅" label="Sunrise"       value={fmtTime(sunrise)}              accent="#ffca28" />
              <MetricCard icon="🌇" label="Sunset"        value={fmtTime(sunset)}                accent="#fb923c" />
              <MetricCard icon="💨" label="Max Wind"      value={fmtVal(windMax, 1)} unit="km/h" accent="#69ff47" />
              <MetricCard icon="💧" label="Humidity"      value={fmtVal(humidity, 0)} unit="%" accent="#00e5ff" />
              <MetricCard icon="☀️" label="UV Index"      value={fmtVal(uvIndex, 1)}             accent="#ffca28" />
              <MetricCard icon="🌂" label="Precip Prob"   value={fmtVal(precipProbMax, 0)} unit="%" accent="#f48fb1" />
            </div>
          </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-3">
              <div className="glass-card p-4 col-span-2 sm:col-span-1 flex flex-col gap-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-semibold tracking-widest uppercase text-text-muted" style={{ fontFamily: "'Exo 2',sans-serif" }}>AQI (EU)</span>
                </div>
              ...
              <MetricCard icon="🟠" label="PM10"   value={fmtVal(pm10, 1)}  unit="µg/m³" accent="#ff6b6b" />
              <MetricCard icon="🟡" label="PM2.5"  value={fmtVal(pm25, 1)}  unit="µg/m³" accent="#ffca28" />
              <MetricCard icon="💨" label="CO"     value={fmtVal(co, 0)}    unit="µg/m³" accent="#b388ff" />
              <MetricCard icon="🔴" label="CO₂"    value="N/A"              sub="Not in API" accent="#546e7a" />
              <MetricCard icon="🟤" label="NO₂"    value={fmtVal(no2, 1)}   unit="µg/m³" accent="#fb923c" />
              <MetricCard icon="🟣" label="SO₂"    value={fmtVal(so2, 1)}   unit="µg/m³" accent="#f48fb1" />
            </div>
          </div>
